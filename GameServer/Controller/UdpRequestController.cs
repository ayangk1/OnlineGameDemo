...
{
    public class UdpRequestController : Singleton<UdpRequestController>
    {
        public delegate void OnActionHandler(ref UdpClient client, byte[] buffer);
        ...
        //派发请求
        public void Dispatch(ushort protoCode, UdpClient client, byte[] buffer)
        {
            if (dic.ContainsKey(protoCode))
            {
                List<OnActionHandler> lstHandler = dic[protoCode];
                if (lstHandler.Count > 0 && lstHandler != null)
                {
                    for (int i = 0; i < lstHandler.Count; i++)
                    {
                        if (lstHandler[i] != null)
                            lstHandler[i](ref client, buffer);
                    }
                }
            }
        }
